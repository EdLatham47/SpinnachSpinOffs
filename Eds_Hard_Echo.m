...
if ~isfield(parameters,'t_last')
    error('time to sample around the echo must be specificed in parameters.tc variable.');
end
if (~isnumeric(parameters.t_last))||(~isreal(parameters.t_last))||...
   (numel(parameters.t_last)~=1)||(parameters.t_last<=0)
    error('parameters.t_last must be a positive number.');
end
...
if (0.5*parameters.t_last>parameters.ta)||(parameters.tb>parameters.ta)
